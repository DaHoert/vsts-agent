...
namespace Microsoft.VisualStudio.Services.Agent.Worker.Build
{
    ...
    public sealed class BuildDirectoryManager : AgentService, IBuildDirectoryManager, IMaintenanceServiceProvider
    {
            ...
            WorkspaceOptions workspace)
        {
            ...
            if (existingConfig == null)
            {
                ...
                newConfig = trackingManager.Create(
                    ...
                    hashKey,
                    trackingFile,
                    repository.TestOverrideBuildDirectory(HostContext.GetService<IConfigurationStore>().GetSettings()));
                ArgUtil.NotNull(newConfig, nameof(newConfig));
            }
            else
            {
        ...
        private BuildCleanOption GetBuildDirectoryCleanOption(IExecutionContext executionContext, WorkspaceOptions workspace)
        {
            ...
            else if (string.Equals(workspace.Clean, PipelineConstants.WorkspaceCleanOptions.ResourceDir, StringComparison.OrdinalIgnoreCase))
            {
                return BuildCleanOption.Source;
            }
            else if (string.Equals(workspace.Clean, PipelineConstants.WorkspaceCleanOptions.OutputDir, StringComparison.OrdinalIgnoreCase))
            {
